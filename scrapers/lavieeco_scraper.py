"""Scraper for La Vie Eco (lavieeco.com) business news site."""
import logging
import re
...
class LavieEcoScraper(BaseScraper):
    """Scraper for https://lavieeco.com articles."""

    BASE_URL = "https://lavieeco.com"

    def __init__(self, supabase_client: Client, rate_limit: float = 3.0):
        super().__init__("La Vie Eco", supabase_client, rate_limit=rate_limit)
        self.categories = ["/affaires", "/economie", "/entreprises"]
    ...
    def get_article_urls(self) -> List[str]:
                ...
                try:
                    ...
                    for article_elem in soup.find_all("article"):
                        ...
                        if link:
                            ...
                            if href not in urls and "lavieeco.com" in href:
                                urls.append(href)
                                found_on_page += 1
                    ...
                    if found_on_page == 0:
                        for heading in soup.find_all(["h2", "h3"]):
                            ...
                            if link:
                                ...
                                if not href.startswith("http"):
                                    href = self.BASE_URL + href
                                if href not in urls and "lavieeco.com" in href and "/category/" not in href:
                                    urls.append(href)
                                    found_on_page += 1
        ...
        logger.info(f"Found {len(urls)} article URLs from La Vie Eco")
        return urls[:50]
    ...
    def scrape_article(self, url: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            if not article_text or len(article_text) < 100:
                for sel in ["div.entry-content", "div.post-content", "div.article-body", "div.td-post-content", "main"]:
                    elem = soup.select_one(sel)
                    if elem:
