"""Scraper for Medias24 news site."""
import logging
...
class Medias24Scraper(BaseScraper):
    ...
    def get_article_urls(self) -> List[str]:
        """
        Get list of article URLs from Medias24.
        ...
        for section in self.sections:
            for page in range(1, max_pages + 1):
                try:
                    ...
                    if found_on_page == 0:
                        logger.warning(f"No articles found on {section_url} with any selector")

                except Exception as e:
                    logger.error(f"Error fetching {section} page {page}: {str(e)}")
                    continue

        logger.info(f"Found {len(urls)} article URLs from Medias24")
        return urls[:50]  # Limit to 50 URLs

    def scrape_article(self, url: str) -> Optional[Dict[str, Any]]:
        """
        Scrape a single article from Medias24.
        ...
        try:
            ...
            # Extract title
            title = None
            for selector in ["h1.article-title", "h1", "div.article-header h1"]:
            ...
            published_date = None
            for selector in ["time", "span.article-date", "div.publish-date", "span[datetime]"]:
                elem = soup.select_one(selector)
                if elem:
            ...
            article_text = None
            for selector in ["article div.article-content", "article", "div.article-body", "div.content"]:
                elem = soup.select_one(selector)
                if elem:
